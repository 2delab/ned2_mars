...
class JointStatePrefixer(Node):
    ...
    def __init__(self):
        ...
        self.joint_states: Dict[str, Optional[JointState]] = {ns: None for ns in self.namespaces}
    ...
    def _setup_trajectory_proxy(self):
        ...
        for ns in self.namespaces:
            ...
            self.traj_servers[ns] = ActionServer(
                ...
                execute_callback=lambda gh, namespace=ns: self._execute_trajectory(gh, namespace),
    ...
    def _strip_prefix(self, trajectory: JointTrajectory, namespace: str) -> JointTrajectory:
    ...
    def _execute_trajectory(self, goal_handle, namespace: str):
        ...
        try:
            ...
            hw_trajectory = self._strip_prefix(goal_handle.request.trajectory, namespace)
            ...
            if error_msg or not hw_goal_handle:
                self.get_logger().error(f"[{namespace}] Failed to send goal: {error_msg}")
                goal_handle.abort()
                return self._create_result(FollowJointTrajectory.Result.INVALID_GOAL)
            ...
            self.get_logger().info(f"[{namespace}] Goal accepted, waiting for result...")
            ...
            while not result_event.is_set():
                ...
                if goal_handle.is_cancel_requested:
                    ...
                    return self._create_result(FollowJointTrajectory.Result.PATH_TOLERANCE_VIOLATED)
                
                # Check for timeout
                elapsed = (self.get_clock().now() - start_time).nanoseconds / 1e9
                if elapsed > self.traj_timeout:
                    self.get_logger().error(f"[{namespace}] Execution timeout ({elapsed:.1f}s)")
                    hw_goal_handle.cancel_goal_async()
                    goal_handle.abort()
                    return self._create_result(FollowJointTrajectory.Result.GOAL_TOLERANCE_VIOLATED)
            ...
            if result_error or not hw_result:
                self.get_logger().error(f"[{namespace}] Failed to get result: {result_error}")
                goal_handle.abort()
                return self._create_result(FollowJointTrajectory.Result.INVALID_GOAL)
            
            # Set goal state and return
            if hw_result.error_code == FollowJointTrajectory.Result.SUCCESSFUL:
                self.get_logger().info(f"[{namespace}] Trajectory completed successfully")
                goal_handle.succeed()
            else:
                self.get_logger().warn(f"[{namespace}] Trajectory failed with code {hw_result.error_code}")
                goal_handle.abort()
